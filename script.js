...
// const submitOrderPopup = document.getElementById("submitOrderPopup");

// Open popup instead of old prompts
// ===================== PLACE ORDER =====================
function placeOrder() {
  if (cart.length === 0) {
    alert("🛒 Your cart is empty!");
...
// Submit order from popup
submitOrderPopup.addEventListener("click", () => {
  const tableNo = document.getElementById("tableNumber").value.trim();
  const customerName = document.getElementById("customerName").value.trim();
  const mobile = document.getElementById("mobileNumber").value.trim();

  if (!tableNo || !customerName || !mobile) {
    alert("Please fill all fields!");
    return;
  }

  if (!/^\d{11}$/.test(mobile)) {
    alert("Mobile number must be 11 digits and numeric!");
    return;
  }
...
document.addEventListener("click", function(e) {
  if (e.target.id === "openReviewPopup") {
    e.preventDefault();
...
// ⭐ STAR CLICK SYSTEM (ADD HERE)
const stars = document.querySelectorAll("#starRating span");
const starInput = document.getElementById("reviewStar");
...
document.getElementById("submitReview").addEventListener("click", () => {
  ...
  const star = parseInt(document.getElementById("reviewStar").value) ||5;
...
document.querySelectorAll(".nav-left .dropdown a").forEach(link => {
  if (link.innerText.toLowerCase() === "reviews") {
